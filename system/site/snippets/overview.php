...
if( $page > -1 ) $posts = $posts->paginate( $page );

foreach( $posts->get() as $post ) snippet( 'post', array( 'post' => $post ) );

$prev_page = false;
if( $page > 1 ) $prev_page = true;

$next_page = true;
if( count($posts->get()) < $eigenheim->config->get('posts_per_page') ) {
...
if( $show_pagination ) {
	...
	<nav class="pagination">
		<ul>
			<?php
			if( $prev_page ) {
				echo '<li><a href="'.url($pre_url.'page/'.($page-1)).'" rel="prev">&laquo; previous page</a></li>';
			} else {
				echo '<li>&laquo; previous page</li>';
			}
			
			if( $next_page ) { // TODO: find out if there are more posts on the next page
				echo '<li><a href="'.url($pre_url.'page/'.($page+1)).'" rel="next">next page &raquo;</a></li>';
			} else {
